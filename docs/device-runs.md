...
- It **is** a gap between what loopty accepts and what it can run, and it is now
  ...
  other reductions loopy will not realize, loopy's rules for numbering and
  sharing hardware axes, loops it unrolls or vectorizes without a numeric
  length, hardware axes on the C target, and loop orders loopy cannot keep;
  see notes 11, 14 and 6 in `docs/loopy-notes.md`), and no others, so a
  schedule it passes can still fail in code generation for a reason nobody has
  met.

...
the restatement is worth no more than it. `loopty run` compiles `solve` too, as
one kernel: its term is the two kernels' statements in call order, and the
compiled program is compared with the program run natively.
